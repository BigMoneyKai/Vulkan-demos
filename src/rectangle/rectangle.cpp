...
int main() {
    ...
    appInfo.pApplicationName = "Vulkan rectangle";
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 960, "rectangle", nullptr, nullptr);
    ...
    void* vertexData;
    vkMapMemory(device, vertexBufferDeviceMemory, 0, vertexBufferCreateInfo.size, 0, &vertexData);
    memcpy(vertexData, vertices.data(), vertexBufferCreateInfo.size);
    ...
    indexBufferMemoryAllocateInfo.allocationSize = indexBufferCreateInfo.size;
    indexBufferMemoryAllocateInfo.memoryTypeIndex = findMemoryType(
        physicalDevice, 
        indexBufferMemoryRequirements.memoryTypeBits,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
    );
    vkAllocateMemory(device, &indexBufferMemoryAllocateInfo, nullptr, &indexBufferDeviceMemory);

    void* indexData;
    vkMapMemory(device, indexBufferDeviceMemory, 0, indexBufferCreateInfo.size, 0, &indexData);
    memcpy(indexData, indices.data(), indexBufferCreateInfo.size);
    vkUnmapMemory(device, indexBufferDeviceMemory);
    vkBindBufferMemory(device, indexBuffer, indexBufferDeviceMemory, 0);
    ...
    const std::vector<uint32_t> vertShaderSrcCode = readShader("shaders/spv/rectangle/rectangle.vert.spv");
    const std::vector<uint32_t> fragShaderSrcCode = readShader("shaders/spv/rectangle/rectangle.frag.spv");
